...
const int yPin = A1;        // Y pin on accelerometer is connected to Arduino's Analog Pin 4
...
void readPotentiometer(){
  ...
  potVal = 200;//map(analogRead(potPin), 0, 1023 , 0, 255);
...
void adjustSpeed(){
  ...
  LEDAccel = constrain(map(analogRead(yPin), 230, 1023 , maxLEDSpeed, -maxLEDSpeed),-maxLEDSpeed, maxLEDSpeed);
